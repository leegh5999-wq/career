...
## 2026-07-08 — Phase 5 완료 (자격·경력기간)
- **범위 결정**: 기술등급 자동 계산은 하지 않음 — 협회별 점수표가 다르고 개정이 잦아 오답 위험 > 효용. 산정 재료(중복 제거 경력기간 + 자격 목록)만 정확히 제공.
- **경력기간 합산**: src/lib/career.ts — 구간 병합(mergeIntervals), 시작일 포함(+1일), 종료일 없는 진행중 프로젝트는 오늘까지. E2E에서 수치 검증 완료(겹치는 3건 중복 제거 확인).
